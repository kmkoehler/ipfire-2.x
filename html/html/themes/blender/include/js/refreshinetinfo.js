...
function refreshInetInfo() {
	...
	$.ajax({
		...
		success: function(xml){
			...
			var conn	= $("conn",xml).text();
			var uptime	= $("uptime",xml).text();
			
			conn = conn.replace(/#/g, "<br/>");
			var auptime = uptime.split("#");
			
			$("#inetConnection").html(conn);
			$("#serverTime").html(auptime[0]);
			$("#serverUptime").html(auptime[1]+'<br/>'+auptime[2]);
			$("#serverLoad").html(auptime[3]+'<br/>'+auptime[4]+' '+auptime[5]+' '+auptime[6]);
